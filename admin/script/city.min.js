...
var City=function(){
    ...
    this.getCities=function(province){
        if(province.length==0){
            console.error('Please input province name');return
        }
    ...
    this.showProvinces = function(element){
        var str='<option value="0">Select Province</option>';
        for(let i in this.getProvinces()){
            if (this.p[i] == this.vprovince) { 
                this.psvar = ' selected'; 
            } else { 
                this.psvar = ''; 
            } 
            str+='<option value="'+this.p[i]+'"' + this.psvar + '>'+this.p[i]+'</option>'
        }
        this.p=[];
    ...
    this.showCities=function(province,element){
        var str='<option value="0">Select City/Municipality</option>';
        var elem='';
        if((province.indexOf(".") !== -1 || province.indexOf("#") !== -1)){
            ...
            for(let i in this.getCities(province)){
                if (this.c[i] == this.vcity) { 
                    this.csvar = ' selected'; 
                } else { 
                    this.csvar = ''; 
                } 
                str+='<option value="'+this.c[i]+'"' + this.csvar + '>'+this.c[i]+'</option>'
            }
            elem=element
        }
        this.c=[]; 
        ...
        if (this.vprovince) {
            var Obj2 = new City();
            Obj2.vcity = this.vcity;
            Obj2.showCities(this.vprovince, elem);
        }
        return this
    }
}